...
const skills = computed(() =>
    Array.isArray(attributes.skills) ? attributes.skills : [],
);
const experience = computed(() =>
    Array.isArray(attributes.experience) ? attributes.experience : [],
);
const education = computed(() =>
    Array.isArray(attributes.education) ? attributes.education : [],
);
const training = computed(() =>
    attributes.training &&
    typeof attributes.training === "object" &&
    !Array.isArray(attributes.training)
        ? attributes.training
        : {},
);
...
function forceLightForPrint() {
    prevTheme = document.documentElement.classList.contains("dark")
        ? "dark"
        : "light";
...
onMounted(async () => {
    ...
    if (mql && "addEventListener" in mql) {
        mql.addEventListener("change", (e) =>
            e.matches ? forceLightForPrint() : restoreThemeAfterPrint(),
        );
    }
});
...
<template>
    <div
        class="min-h-screen bg-slate-50 text-slate-800 dark:bg-slate-950 dark:text-slate-100"
    >
        <!-- Topbar (não imprime) -->
        <div
            class="sticky top-0 z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur border-b border-slate-200 dark:border-slate-800 px-4 print:hidden"
        >
            <div
                class="mx-auto max-w-3xl py-3 flex items-center justify-between gap-3"
            >
                <!-- Avatar + nome + badge -->
                <div class="flex items-center gap-3 min-w-0">
                    <div
                        class="w-9 h-9 flex items-center justify-center rounded-full bg-gradient-to-br from-sky-600 to-indigo-600 text-white font-bold shadow-sm ring-2 ring-white/70 dark:ring-slate-800 select-none"
                    >
                        {{ initials }}
                    </div>
                    <div class="leading-tight min-w-0">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            >
                            <span
                                class="px-2 py-0.5 text-[10px] rounded-full bg-sky-100 text-sky-700 dark:bg-sky-900/60 dark:text-sky-300 whitespace-nowrap"
                            >
                                {{ roleBadge }}
                            </span>
                        </div>
                ...
                <div class="flex items-center gap-3">
                    <button
                        class="px-3 py-1.5 rounded-xl border border-slate-300 hover:bg-slate-100 text-sm dark:border-slate-600 dark:hover:bg-slate-800"
                        @click="toggleTheme"
        ...
        <!-- Página -->
        <main
            class="mx-auto max-w-3xl p-6 md:p-10 my-6 bg-white shadow-sm rounded-2xl dark:bg-slate-900 dark:shadow-none print:shadow-none print:rounded-none"
        >
            <!-- Cabeçalho -->
            <header
                class="grid grid-cols-1 md:grid-cols-[1fr_auto] items-start gap-4 print:grid-cols-[1fr_auto] print:gap-4"
            >
                <div>
                    <h1
                        class="text-3xl font-bold tracking-tight text-sky-700 dark:text-sky-400"
                    >
                        {{ name }}
                    </h1>
                    <p v-if="title" class="text-slate-600 dark:text-slate-300">
                        {{ title }}
                    </p>
                </div>
                <ul
                    class="text-sm text-slate-600 dark:text-slate-300 md:text-right leading-6"
                >
                    ...
                    <li v-if="links.linkedin" class="whitespace-nowrap">
                        ...
                        <a
                            class="link break-all md:break-normal"
            ...
            <section v-if="profile">
                ...
                <p
                    class="mt-2 text-[15px] leading-relaxed text-slate-700 dark:text-slate-300"
                >
                    {{ profile }}
                </p>
            </section>
            ...
            <section v-if="skills.length" class="mt-6">
                ...
                <div class="mt-3 flex flex-wrap gap-2">
                    <span v-for="(s, i) in skills" :key="i" class="pill">{{
                        s
                    }}</span>
                </div>
            </section>
            ...
            <section v-if="experience.length" class="mt-6">
                ...
                <div class="mt-3 space-y-5">
                    ...
                    >
                        <header
                            class="flex items-baseline justify-between gap-3 flex-wrap sm:flex-nowrap"
                        >
                            <div class="min-w-0">
                            ...
                            <span v-if="exp.sector" class="pill !text-[11px]">{{
                                exp.sector
                            }}</span>
                        </header>
                        ...
                        >
                            <li v-for="(b, j) in exp.bullets" :key="j">
                                {{ b }}
                            </li>
                        </ul>

                        <div
                            v-if="Array.isArray(exp.stack) && exp.stack.length"
                            class="mt-2 flex flex-wrap gap-2"
                        >
                            <span
                                v-for="(tech, j) in exp.stack"
                                :key="j"
                                class="pill"
                                >{{ tech }}</span
                            >
            ...
            <section v-if="Object.keys(training).length" class="mt-6">
                <h2 class="text-lg font-semibold">
                    Formação Profissional (seleção)
                </h2>
                <div
                    class="mt-3 grid md:grid-cols-2 gap-4 text-[15px] leading-relaxed"
                >
                    <div v-if="Array.isArray(training.dev)">
                        <h3 class="font-medium">Desenvolvimento / Web</h3>
                        <ul class="list-disc pl-5 mt-1">
                            <li v-for="(t, i) in training.dev" :key="i">
                                {{ t }}
                            </li>
                        </ul>
                    </div>
                    <div v-if="Array.isArray(training.db)">
                        ...
                        <ul class="list-disc pl-5 mt-1">
                            <li v-for="(t, i) in training.db" :key="i">
                                {{ t }}
                            </li>
                        </ul>
                    </div>
                    <div v-if="Array.isArray(training.agile)">
                        ...
                        <ul class="list-disc pl-5 mt-1">
                            <li v-for="(t, i) in training.agile" :key="i">
                                {{ t }}
                            </li>
                        </ul>
                    </div>
